...
mdxscid_data <- adata[,adata$genotype == "MdxSCID"]
dim(mdxscid_data)

# Log-transform the normalized counts
normalized_counts <- assay(mdxscid_data, "sqrt_norm") 

# Perform log transformation
logcounts <- log1p(normalized_counts)

# Add the log-transformed counts as a new assay
assay(mdxscid_data, "logcounts") <- logcounts
...
pred <- SingleR(test = mdxscid_data, ref = ref, labels = translated)

# Convert prediction to DataFrame and assign to adata
pred_df <- as.data.frame(pred)
mdxscid_data$SingleR <- pred_df$labels

# Save annotated data back to h5ad format
writeH5AD(mdxscid_data,'/Users/srivalli/Documents/GitHub/hofmann_dmd/data/annotated_mdxscid_ont_data.h5ad')